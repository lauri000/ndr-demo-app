...
#[test]
fn legacy_create_wire_is_ignored() -> Result<()> {
    let alice = manager_device(29, 231);
    let bob = manager_device(30, 241);
    ...
    let event = groups.handle_incoming(bob.owner_pubkey, &payload)?;
    assert!(event.is_none());
    assert!(groups.group("legacy-group").is_none());
    Ok(())
}

#[test]
fn missing_wire_version_or_protocol_is_ignored() -> Result<()> {
    let alice = manager_device(31, 251);
    let bob = manager_device(32, 252);
    let mut groups = GroupManager::new(alice.owner_pubkey);

    let missing_wire_version = serde_json::to_vec(&serde_json::json!({
        "payload": {
            "kind": "create_group",
            "group_id": "missing-wire-version",
            "protocol": "pairwise_fanout_v1",
            "base_revision": 0,
            "new_revision": 1,
            "name": "MissingWireVersion",
            "created_by": bob.owner_pubkey,
            "members": [bob.owner_pubkey, alice.owner_pubkey],
            "admins": [bob.owner_pubkey],
            "created_at": 1_900_001_000u64,
    ...
    assert!(groups
        .handle_incoming(bob.owner_pubkey, &missing_wire_version)?
        .is_none());
    assert!(groups.group("missing-wire-version").is_none());

    let missing_protocol = serde_json::to_vec(&serde_json::json!({
        "wire_format_version": 1,
        "payload": {
            "kind": "create_group",
            "group_id": "missing-protocol",
            "base_revision": 0,
            "new_revision": 1,
    ...
    assert!(groups
        .handle_incoming(bob.owner_pubkey, &missing_protocol)?
        .is_none());
    assert!(groups.group("missing-protocol").is_none());
    Ok(())
}
